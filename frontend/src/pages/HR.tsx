    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-[32px] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative border border-white/20">
        ...
        <div className="p-8 text-left">
          <h2 className="text-[24px] font-black text-[#3d1a2e] mb-2" style={{ fontFamily: "var(--font-heading)" }}>Bảng Tính Lương Chi Tiết</h2>
    ...
    <div className="p-8 min-h-screen relative overflow-hidden">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-[#3d1a2e] text-[28px] font-bold" style={{ fontFamily: "var(--font-heading)" }}>Nhân Sự & Chấm Công</h1>
        </div>
        <button 
      ...
      <div className="flex gap-2 mb-6">
        {[
          ...
          <button 
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-2xl transition-all font-bold text-[13px] shadow-sm hover:shadow-md active:scale-95" 
            style={{ 
              background: activeTab === t.key ? "linear-gradient(135deg, #D4AF37, #C9A94E)" : "rgba(255,255,255,0.7)", 
              color: activeTab === t.key ? "white" : "#6b4153", 
              border: "1px solid", 
              borderColor: activeTab === t.key ? "transparent" : "rgba(255,255,255,0.9)", 
              boxShadow: activeTab === t.key ? "0 8px 24px rgba(212,175,55,0.25)" : "none" 
            }}
          >
      ...
      {isLoading ? (
        ...
        <div style={glassCard} className="p-6">
          <div className="flex items-center gap-3 mb-6 p-4 bg-[#fdfbf7] border border-[#D4AF37]/20 rounded-2xl">
            ...
            <div>
              <p className="text-[#3d1a2e] font-bold text-[14px]">Bảng lương tháng {new Date().getMonth() + 1}</p>
              <p className="text-[#9d6b7a] text-[12px]">Chuẩn công: 26 ngày/tháng. Lương thực nhận = (Lương CB / 26 * Công) + Tăng ca.</p>
            </div>
          </div>
                  ...
                  <tr key={emp.id} className="border-b border-[rgba(212,175,55,0.07)] hover:bg-[rgba(212,175,55,0.02)]">
                    ...
                    <td className="py-4 text-center">
                      ...
                      {emp.displayStatus === 'Chưa thanh toán' && (
                        <button disabled className="px-4 py-1.5 rounded-lg bg-amber-50 text-amber-700 text-[11px] font-bold border border-amber-200/50 cursor-not-allowed">Đã chốt - Chờ chi (Tài chính)</button>
                      )}
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <h2 className="text-xl font-bold mb-6 text-[#3d1a2e]" style={{ fontFamily: "var(--font-heading)" }}>Thêm Nhân Viên Mới</h2>
            <div className="space-y-4">
              ...
              <button onClick={handleSaveEmployee} disabled={isSubmitting} className="w-full mt-4 py-4 rounded-2xl font-bold text-white shadow-lg transition-all hover:scale-105 active:scale-95 disabled:bg-gray-300 disabled:scale-100" style={{ background: "linear-gradient(135deg, #D4AF37, #C9A94E)" }}>{isSubmitting ? "Đang xử lý..." : "Lưu Thông Tin"}</button>
            </div>
          </div>
        </div>
      )}
